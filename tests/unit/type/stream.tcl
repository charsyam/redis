...
start_server {
    ...
    test {XTRIM with KEEPREF/ACKED triggers stream node GC correctly} {
        set old_node_max_entries [lindex [r config get stream-node-max-entries] 1]
        r config set stream-node-max-entries 100

        try {
            # ACKED skips entries still referenced by a group, so the tombstones
            # are not a contiguous prefix of the node.
            r del somestream
            r del somestream-copy
            r xgroup create somestream grp $ mkstream
            for {set j 0} {$j < 40} {incr j} {
                r xadd somestream 1-[expr {$j+1}] item $j
            }
            # Read all, then acknowledge only the even-indexed entries.
            r xreadgroup group grp c1 count 40 streams somestream >
            for {set j 0} {$j < 40} {incr j 2} {
                r xack somestream grp 1-[expr {$j+1}]
            }
            r xtrim somestream maxlen 0 acked

            set forward [r xrange somestream - +]
            set reverse [r xrevrange somestream + -]
            assert_equal [llength $forward] [r xlen somestream]
            assert_equal [lreverse $forward] $reverse
            # Only the unacknowledged (odd-indexed) entries must survive.
            foreach entry $forward {
                set fields [lindex $entry 1]
                assert_equal 1 [expr {[lindex $fields 1] % 2}]
            }

            set payload [r dump somestream]
            r restore somestream-copy 0 $payload
            assert_equal $forward [r xrange somestream-copy - +]
        } finally {
            r del somestream
            r del somestream-copy
            r config set stream-node-max-entries $old_node_max_entries
